...
sed 's/{{CERT}}/'$CA_CRT'/g' 01-update-secret.tpl > 01-update-secret.yaml

echo "Aplicando manifiesto"
#kubectl apply -f 01-update-secret.yaml

docker run --rm --name kubectl -v $PWD/kubeconfig:/.kube/config -v $PWD/01-update-secret.yaml:/01-update-secret.yaml bitnami/kubectl:1.16.3 apply -f 01-update-secret.yaml
